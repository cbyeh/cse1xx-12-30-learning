...
public class Explorer {
    ...
    public void solve() {
        ...
        while (!gameOver() && !isEmpty()) {
            startingSquare = step();
            if (startingSquare.hasNemo()) {
                setPath(startingSquare);
                setFoundNemo();
                setGameOver();
            }
        }
    ...
    public Square step() {
        ...
        this.getSea().setVisit(currentSquare.getRow(), currentSquare.getCol());
        ArrayList<Square> neighbors = this.getSea().getAdjacentArea(currentSquare);
        for (Square sq : neighbors) {
            if (!sq.isVisited()) {
                sq = new Square(sq.getRow(), sq.getCol(), sq.getType());
                sq.setPrevious(currentSquare);
                exploreList.add(sq);
            }
        }
        /* ArrayList<Square> neighbors = this.getSea().getAdjacentArea(currentSquare);
...
// from start to finish.
    public void setPath(Square finish) {
        ...
        StackExploreList myList = new StackExploreList();
        while (currentSquare != null) { // entire path
            sea.getSea()[currentSquare.getRow()][currentSquare.getCol()].setFinalPath();
            myList.add(currentSquare);
            currentSquare = currentSquare.getPrevious();
        }
        while (!myList.isEmpty()) {
            Square addToPath = myList.getNext();
            path += squareLocation(addToPath.getRow(), addToPath.getCol());
        }
